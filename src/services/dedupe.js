 ...
 * enquiry to the webhook again. This is the backstop: same email + requirement
...
export function fingerprintLead(lead) {
  const basis = [lead?.email?.trim().toLowerCase(), lead?.requirement?.trim().toLowerCase().slice(0, 200)]
    .filter(Boolean)
    .join('|');
